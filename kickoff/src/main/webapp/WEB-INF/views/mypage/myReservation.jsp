			...
			<div class="inner">
				...
				<header id="header">
					...
					<div class="wrap">
						...
						<div class="count-text">총 예약건수<b class="count">${listCnt}</b></div>
					</div>
				</header>
				<c:choose>
										...
										<div class="container p-0">
											<div class="root-content">
												<div class="container">
													<div class="facility-content">
														<div class="image-wrap">
															<img class="image" src="${row.IMG_PATH}${row.IMG_NAME}" />
														</div>
														<div class="facility-wrap">
															<h2>${row.RESERVATION_PLACE_NAME}</h2>
															<h3>${row.RESERVATION_PLACE_ADDRESS}</h3>
															<p class="info">
																예약 날짜<span>${row.RESERVATION_DATE}</span>
															</p>
															<p class="info">
																예약 시간<span>${row.RESERVATION_START_TIME}~${row.RESERVATION_END_TIME}</span>
															</p>
															<p class="info">
																대관 비용<span>${row.RESERVATION_PRICE}원</span>
															</p>
															<p class="info">
																예약 상태<span class="status">${row.USER_STATUS}</span>
															</p>
														</div>
													</div>
												</div>
												<div class="container">
													<div class="map-wrap">
														...
														<script>
															...
															// 주소로 좌표를 검색합니다
															geocoder${num.index}.addressSearch('${row.RESERVATION_PLACE_ADDRESS}', function(result, status) {
												...
												<div class="btn-container">
													<a href="javascript:applyCancel('/applyCancel?reservationNo=${row.RESERVATION_ORIGIN_NO}')" id="cancle" class="cancle">예약 취소</a>
													<a href="/reservationDetail?reservationNo=${row.RESERVATION_ORIGIN_NO}" id="detail" class="detail">상세 보기</a>
												</div>
											</div>
										</div>
									</div>
				...
				<div class="paging-wrap container">
					<div class="paging-container">
						<c:if test="${pagination.curPage ne 1}">
				    		<a href="/myReservation?curPage=${pagination.prevPage}" class="pageText">&lt;Prev</a>
				    	</c:if> 
						<c:forEach var="pageNum" begin="${pagination.startPage}" end="${pagination.endPage}">
				        	<c:choose>
				           		<c:when test="${pageNum eq pagination.curPage}">
				               		<a href="/myReservation?curPage=${pageNum}" class="pageText">${pageNum}</a> 
				           		</c:when>
				           		<c:otherwise>
				               		<a href="/myReservation?curPage=${pageNum}" class="pageText">${pageNum}</a> 
				           		</c:otherwise>
				   		...
				   		<c:if test="${pagination.curPage ne pagination.pageCnt && pagination.pageCnt > 0}">
				   			<a href="/myReservation?curPage=${pagination.nextPage}" class="pageText">Next&gt;</a>
				   		</c:if> 
				   	</div>
				</div>
				<!-- Paging END -->
